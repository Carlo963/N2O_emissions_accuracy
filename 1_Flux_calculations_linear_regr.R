...
# insert transparency (alpha) varying with flux magnitude
fluxes.9$Alpha  <- (abs(fluxes.9$g.N2O.N.ha.day)/120)^0.4
...
p1 <-
    ...
    #scale_x_continuous(breaks = seq(-75, 80, 2.5))+
    geom_point(aes(x = g.N2O.N.ha.day, y = R2, , alpha = Alpha), shape = ".")+
    #xlab(expression(paste("g ", N[2], "O-N ", ha^{-1}, day^{-1})))+
    ylab(expression( R^{2}))
...
p2 <-
    ...
    geom_point(aes(x = g.N2O.N.ha.day, y = NRMSE, alpha = Alpha), shape = ".")+
...
p4 <- 
    ...
    xlab(expression(paste("Flux [g ", N[2], "O-N ", ha^{-1}, day^{-1}, "]")))+
    ylab("RCIW")+
    geom_point(aes(x = g.N2O.N.ha.day, y = Abs.rel.confint.amplitude, alpha = Alpha), shape = ".")
...
p5 <-
    ...
    #scale_x_continuous(breaks = seq(-75, 80, 2.5))+
    geom_point(aes(x = g.N2O.N.ha.day, y = RMSE, alpha = Alpha), shape = ".")+
    #xlab(expression(paste("g ", N[2], "O-N ", ha^{-1}, day^{-1})))+
...
p6 <-
    ...
    geom_point(aes(x = g.N2O.N.ha.day, y = Confint95.amplitude, alpha = Alpha), shape = ".")+
    #xlab(expression(paste("g ", N[2], "O-N ", ha^{-1}, day^{-1})))+
